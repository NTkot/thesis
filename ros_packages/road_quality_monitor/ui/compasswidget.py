...
class Compasswidget(QtWidgets.QLabel):
    ...
    def drawMarkings(self, painter : QPainter):
    ...
    def drawNeedle(self, painter : QPainter):
    ...
    def drawOuterCircle(self, painter : QPainter):
        painter.save()
        painter.setPen(QPen(QColor(200, 200, 200), 2.0))

        if self.width() <= self.height():
            widget_diameter = self.width()
        else:
            widget_diameter = self.height()

        painter.drawEllipse(QPointF(self.width() / 2,self.height() / 2), int(widget_diameter / 2), int(widget_diameter / 2))
